...
def test_generate_data():
    assert len(generate_data(num_of_orders=100)[1]) == 100
    assert len(generate_data(num_of_orders=100)[0]) == default_num_of_customers


def test_transform_data():
    customers_data, orders_data = generate_data(500, 10)
